...
// import { Upload, QrCode } from "lucide-react"; // lucide icons
...
//           qrCodeScanner.stop();
//           window.location.href = `/dashboard?token=${encodeURIComponent(decodedText)}`;
//         },
//         () => setStatus("Scanning...")
//       )
...
//   // Handle uploaded ID card
//   const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
//     if (!e.target.files?.length || !html5QrCode) return;
//     const file = e.target.files[0];

//     try {
//       const result = await html5QrCode.scanFile(file, true);
//       setStatus(`QR Code detected: ${result}`);
//       window.location.href = `/dashboard?token=${encodeURIComponent(result)}`;
//     } catch (err) {
...
//           Please scan your <span className="font-semibold">Employee QR Code</span> to login
//         </p>
...
//         {/* Status */}
//         <p className="mt-6 text-sm font-medium text-[#002C5F] flex items-center justify-center gap-2">
...
export default function LoginPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      window.location.href = `/dashboard?token=${encodeURIComponent(result.data)}`;
    } catch (err) {
      setStatus("❌ Could not detect QR code in uploaded image.");
    }
  };
  ...
  return (
    <div className="flex items-center justify-center bg-gradient-to-br from-[#f8fbff] to-[#e6f0ff] min-h-screen relative">
      ...
      <div className="relative bg-white/80 backdrop-blur-lg shadow-2xl rounded-2xl p-8 w-full max-w-md text-center border border-gray-200 z-10">
        ...
        <p className="text-gray-600 mb-6">
          Scan your <span className="font-semibold">Employee QR Code</span> to login
        </p>
